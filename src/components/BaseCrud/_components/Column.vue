  ...
  <template v-else>
    ...
    <template v-else>
      ...
      <el-table-column v-bind="newCol" :formatter="newCol.formatter" :reserve-selection="selection" v-if="newCol.formatter">
        <template #header="{ column, $index, store }">
          {{ column.label }}
...
import { propsJoinChar, deleteAttrs, getPopoverAttrs, emptyVals, devErrorTips, showMessage } from "@/utils";
...
function getCellText(row: CommonObj, type: string) {
...
//优化创建时间、修改时间列的显示
// function getSplitPropsVal(row: CommonObj, prop: string) {
//   if(!prop) return '-';
//   // let html = "";
//   // if (prop?.includes(propsJoinChar)) {
//   //   const [firstProp, secondProp] = prop.split(propsJoinChar);
//   //   html = `
//   // <div>${row?.[firstProp] || "-"}</div>
//   // <div>${row?.[secondProp] || "-"}</div>
//   // `;
//   // } else {
//   //   html = `${row?.[prop] || "-"}`;
//   // }
//   // return html;
//   // <template v-if="col.prop?.includes(propsJoinChar)">
//   //   <div>
//   //     <BaseRender
//   //       :data="getCreateOrUpdateText(row, col.prop as string, 0) "
//   //     />
//   //   </div>
//   //   <div>
//   //     <BaseRender
//   //       :data="getCreateOrUpdateText(row, col.prop as string, 1) "
//   //     />
//   //   </div>
//   // </template>
//   // <template v-else>
