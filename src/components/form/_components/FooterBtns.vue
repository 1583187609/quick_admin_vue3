...
<template>
  <div class="footer-btns f-c-c f-0 pt-h pb-h">
    ...
    <template v-for="(btn, ind) in newMoreBtns" :key="ind">
      <BaseBtn @click="handleMoreBtns(btn)" :name="btn" :attrs="{ disabled }" v-if="btn.popconfirm" />
      <BaseBtn v-debounce.immediate="() => handleMoreBtns(btn)" :name="btn" :attrs="{ disabled }" v-else />
    </template>
