...
-->
<template>
  <BaseForm class="edit-table f-fs-s-c" v-model="model"  ref="formRef">
    <BaseTable :cols="cols" :rows="rows" class="table"  ref="tableRef">
      <template #default="{row,column:col,$index}">
        <template v-if="col.field">
          <FieldItem :field="col.field"/>
        </template>
      </template>
      ...
      >
        <template #header="{row,column,$index}">
          <span :class="{required: col.required}">{{ col.label }}</span>
        </template>
...
import { ref, reactive, useAttrs, inject, watch } from "vue";
