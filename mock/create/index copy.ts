...
export function createRestfulApis(enName = "user", prefix = "/mock") {
  ...
  return {
    // 新增/批量新增（导入）
    [`POST ${prefix}/${enName}/add`]: (req: CommonObj) => {
    ...
    // 删除/批量删除
    [`DELETE ${prefix}/${enName}/del`]: (req: CommonObj) => {
    ...
    [`PATCH ${prefix}/${enName}/update`]: (req: CommonObj) => {
      const params = getRequestParams(req);
      const { byKey = "id", data = needParam() } = params;
    ...
    // 查询详情
    [`GET ${prefix}/${enName}/detail`]: (req: CommonObj) => {
      const pageParams = getRequestParams(req);
      const { byKey = "id", id, ids } = pageParams;
      const byIds = toArray(id ?? ids);
      if (!byIds.length) return responseData({ code: 1, msg: "请传入 id 或 ids " });
      const queryList = getFilterList(
        list,
        { ids: byIds },
        { ids: ["enums", "same", byKey] },
        false,
        undefined,
        undefined,
        enName
    ...
    [`GET ${prefix}/${enName}/detail`]: (req: CommonObj) => {
      const pageParams = getRequestParams(req);
      const { byKey = "id", id, ids, curr_page, page_size, ...params } = pageParams;
      const byIds = toArray(id ?? ids);
      if (!byIds.length) return responseData({ code: 1, msg: "请传入 id 或 ids " });
      let queryList = getFilterList(
        list,
        { curr_page, page_size, ids: byIds, ...params },
        {
          ids: ["enums", "same", byKey],
          age: ["range", "number"],
          name: ["match", "blur"],
    ...
    [`GET ${prefix}/${enName}/detail`]: (req: CommonObj) => {
      const pageParams = getRequestParams(req);
      const { byKey = "id", id, ids, exports, ...params } = pageParams;
      const byIds = toArray(id ?? ids);
      if (!byIds.length) return responseData({ code: 1, msg: "请传入 id 或 ids " });
      let queryList = getFilterList(
        list,
